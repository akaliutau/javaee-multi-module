...
/** 
 *  This class contains the simple form validation tests, 
 *  just to be sure that validator is working 
 * 
 * @author Alex Kalutov
 * @since Version 1.0
 */
...
/**
 * Before any tests an instance of LocalValidatorFactoryBean class is created and configured.
 * <p>NB: error messages in all test-cases are comparing by code; messages themselves
 *    are not being compared because of isolated testing methodology.
 *  
 *  
 */
@Before
public void setup() {
  ...
  public void testNullDepTitleError() {
	   dep = getDep(); dep.setTitle(null);
       Set<ConstraintViolation<Department>> constraintViolations =     
      		 localValidatorFactory.validate(dep);
	        assertEquals(1, constraintViolations.size());
	        assertEquals("javax.validation.constraints.NotNull.message", constraintViolations
  ...
  public void testMinSizeDepTitleError() {
          dep = getDep(); dep.setTitle("a");
         Set<ConstraintViolation<Department>> constraintViolations =     
        		 localValidatorFactory.validate(dep);
	        assertEquals(1, constraintViolations.size());
	        assertEquals("javax.validation.constraints.Size.message", constraintViolations
	                .iterator().next().getMessage());
  ...
  public void testMaxSizeDepTitleError() {
	  	...
	  	String title="";
	  	for(int i=0;i<26;i++) title.concat(block);
        dep = getDep(); dep.setTitle(title);
         Set<ConstraintViolation<Department>> constraintViolations =     
        		 localValidatorFactory.validate(dep);
	        assertEquals(1, constraintViolations.size());
	        assertEquals("javax.validation.constraints.Size.message", constraintViolations
  ...
  public void testNullEmplNamesError() {
       ...
       Set<ConstraintViolation<Employee>> constraintViolations =     
      		 localValidatorFactory.validate(empl);
  ...
  public void testDateEmplError2() {
       ...
       Set<ConstraintViolation<Employee>> constraintViolations =     
      		 localValidatorFactory.validate(empl);
	        assertEquals(1, constraintViolations.size());
	        assertEquals("javax.validation.constraints.Past.message", constraintViolations
	                .iterator().next().getMessage());
  ...
  public void testSalaryEmplError1() {
       ...
       Set<ConstraintViolation<Employee>> constraintViolations =     
      		 localValidatorFactory.validate(empl);
	        assertEquals(1, constraintViolations.size());
	        assertEquals("javax.validation.constraints.Min.message", constraintViolations
	                .iterator().next().getMessage());
  ...
  public void testSalaryEmplError2() {
       ...
       Set<ConstraintViolation<Employee>> constraintViolations =     
      		 localValidatorFactory.validate(empl);
	        assertEquals(1, constraintViolations.size());
	        assertEquals("javax.validation.constraints.Max.message", constraintViolations
	                .iterator().next().getMessage());
